...
@Component({
  selector: 'qv-item-type-icon',
  standalone: true,
  template: `
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="w-4 h-4 ml-1">
      <path [attr.d]="pathD()" fill="#74c7ec" />
    </svg>
  `,
})
export class ItemTypeIconComponent {
  type = input.required<QuakVimItemType>()

  pathD = computed(() => this.getSvgWithType(this.type()))

  private getSvgWithType(type: QuakVimItemType): string {
    return {
      'tabs':
